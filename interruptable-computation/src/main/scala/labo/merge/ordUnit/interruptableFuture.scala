...
object interruptableFuture extends App {
  ...
  def run(commandLine: String, time: Long, timeOut: Duration = 5 seconds): String = {
    println("running ...")
    ...
    try {
      p = Process("/home/hack/EDLA/C/signalINT", None, "TMPDIR" -> "/tmp")
  ...
  def longComputation(delay: Future[String]): String = {
    var i = 0
    try {
      // isCompleted acts as our interrupted-flag
      while (!delay.isCompleted && i < 10) { print(i); Thread.sleep(400); i = i + 1 }
    } catch {
      case ie: InterruptedException ⇒
        println("INTERRUPTED1"); throw (ie)
      case e: TimeoutException ⇒ println("TIMEOUT1")
    }
    i.toString
  }

  def externalComputation(delay: Future[String]): String = {
    try {
  ...
  //ref OK
  /*  val (f, cancel) = interruptableFuture[Int](future => {
    longComputation(future)
  })
