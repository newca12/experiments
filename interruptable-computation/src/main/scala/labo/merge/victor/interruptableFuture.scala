...
object interruptableFuture extends App {
  ...
  def longComputation(delay: Future[Int]): Int = {
    var i = 0
    try {
      // isCompleted acts as our interrupted-flag
      while (!delay.isCompleted && i < 10) { print(i); Thread.sleep(300); i = i + 1 }
    } catch {
  ...
  def externalComputation(delay: Future[Int]): Int = {
    try {
